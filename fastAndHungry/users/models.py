...
class Address(models.Model):
    ...
    def __str__(self):
        """Get str representation."""
        return '%s. %s, %s' % (self.usuario,self.calle,self.exterior)

    def str_rep(self):
        """Get str representation."""
        return 'Calle %s, Exterior %s, Interior %s\nColonia %s Delegación %s C.P. %s ' % (self.calle, self.exterior, self.interior, self.colonia, self.delegacion, self.codigo_postal)
